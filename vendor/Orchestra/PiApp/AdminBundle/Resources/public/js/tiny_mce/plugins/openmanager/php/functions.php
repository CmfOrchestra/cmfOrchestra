<?php
	...
	function directoryScan($dir, $onlyfiles = false, $onlyDir = false, $fullpath = false, $ignorDirName = array()) {
	...
	function display_gallery_page($files_array, $pageno = 1, $prepath = "", $resultspp = 15, $display = true)
	{
		...
		$thumbimagepath = "/uploads/tinymce/images/thumbs/";
		$imagepath = "/uploads/tinymce/images/";
		
		//get files in folder
		if(isset($_GET['dirfiles']) && !empty($_GET['dirfiles'])) {
			$thumbimagepath = "/uploads/tinymce/".  $_GET['dirfiles'] . "/images/thumbs/";
		} else {
			$thumbimagepath = "/uploads/tinymce/images/thumbs/";
		}
		if(isset($_GET['dirfiles']) && !empty($_GET['dirfiles'])) {
			$imagepath = "/uploads/tinymce/" . $_GET['dirfiles'] . "/images/";
		} else {
			$imagepath = "/uploads/tinymce/images/";
		}		
		...
		//get files in folder
		if(isset($_GET['dirfiles']) && !empty($_GET['dirfiles'])) {
			$relthumbimagepath = $prepath.$_GET['d'] .  $_GET['dirfiles'] . "/images/thumbs/";
		} else {
			$relthumbimagepath = $prepath.$_GET['d'] . "images/thumbs/";
		}
		if(isset($_GET['dirfiles']) && !empty($_GET['dirfiles'])) {
			$relimagepath = $prepath.$_GET['d'] .  $_GET['dirfiles'] . "/images/";
		} else {
